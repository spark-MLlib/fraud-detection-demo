...
import com.ververica.field.dynamicrules.MetricsEvaluator.Descriptors;
...
public class DynamicMetricsCalcFunction
  ...
  public void processElement(
    ...
    if (Rule.WindowType.GROWTH_WINDOW == rule.getWindowType()) {
      ...
      if (rule.getRuleState() == RuleState.ACTIVE) {
        ...
        if (rule.getMetricsOutTags().contains(Rule.MetricsOutTag.InMemoryDB)) {
            // TODO 根据实际修改
            // TenantID:EntityID(Campaign/Agent等)
            String entityKey = "TenantID:" + value.getKey();
            String metricCode = String.valueOf(rule.getMetricsCode());
            String metricValue = String.valueOf(aggregateResult);
  ...
  private void aggregateValuesInState(
    ...
    // TODO: count这类统计方法，最好由AggregatorFunctionType指定
